<section class="content">
  ...
  <div class="row">
    ...
    <div class="col-md-12 col-sm-12">
      ...
      <form role="form" action="<?php echo site_url('../admin/candidate/certificate/index/' . $batch->b_id) ?>" method="post" id="update_student_certificate_details_form">
        <!-- Batch ID -->
        ...
        <div class="card">
          ...
          <div class="card-body">
            <table width="100%" class="datatable_colvisw table table-striped table-bordered table-hover table-sm">
              ...
              <tbody>
                <? //= dd($student_certificate_details[0]); 
                ?>
                <?php if (valArr($student_certificate_details)) { ?>
                  <?php $sl = 1; ?>
                  <?php foreach ($student_certificate_details as $student) { ?>
                    <tr>
                      <!-- cer_id cer_cer_id cer_agency cer_certified cer_date cer_certificate_issued cer_certificate_no -->
                      <td>
                        <?php echo $sl ?>
                        <input name="cer_id[<?= $student->bsm_c_id ?>]" class="form-control <?= $input_height ?>" type="hidden" value="<?php echo $student->cer_id ?>">
                      ...
                      <td>
                        <input name="cer_date[<?= $student->bsm_c_id; ?>]" class="form-control <?= $input_height . " " . (form_error("cer_date[" . $student->bsm_c_id . "]") ? "is-invalid" : null);  ?>" type="date" placeholder="<?php echo ('Date') ?>" id="cer_date_<?= $student->bsm_c_id; ?>" value="<?php echo $student->cer_date ?>">
                      </td>
                      <td>
                        <input name="cer_certificate_issued[<?= $student->bsm_c_id; ?>]" class="form-control <?= $input_height . " " . (form_error("cer_certificate_issued[" . $student->bsm_c_id . "]") ? "is-invalid" : null);  ?>" type="text" placeholder="<?php echo ('Certificate Name') ?>" id="cer_certificate_issued_<?= $student->bsm_c_id; ?>" value="<?php echo $student->cer_certificate_issued ?>">
                      </td>
                      <td>
                        <input name="cer_certificate_no[<?= $student->bsm_c_id; ?>]" class="form-control <?= $input_height . " " . (form_error("cer_certificate_no[" . $student->bsm_c_id . "]") ? "is-invalid" : null);  ?>" type="text" placeholder="<?php echo ('Certificate Number') ?>" id="cer_certificate_no_<?= $student->bsm_c_id; ?>" value="<?php echo $student->cer_certificate_no ?>">
                      </td>
          ...
          <div class="card-footer">
            <button type="submit" name="update_student_assessment_details_form" value="update_student_assessment_details" class="float-right form-control-sm btn btn-warning btn-sm ">
